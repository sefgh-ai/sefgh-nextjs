...
export function useCategories(type = null) {
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            if (payload.new.is_active && (!type || payload.new.type === type)) {
              setCategories(prev => [...prev, payload.new].sort((a, b) => {
            ...
            if (payload.new.is_active && (!type || payload.new.type === type)) {
              ...
              setCategories(prev => prev.filter(cat => cat.id !== payload.new.id))
            }
  ...
  const addCategory = async (name, icon, categoryType = 'custom', description = '') => {
    try {
