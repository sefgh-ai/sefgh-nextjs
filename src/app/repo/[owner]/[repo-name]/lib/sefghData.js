...
export async function fetchSefghData(supabase, repoFullName) {
  try {
    ...
    const [votesData, ratingsData, commentsData, savesData] = await Promise.allSettled([
    ...
    return {
      votes: votesData.status === 'fulfilled' ? votesData.value.votes : { upvotes: 0, downvotes: 0, net_votes: 0 },
      userVote: votesData.status === 'fulfilled' ? votesData.value.userVote : null,
      ratings: ratingsData.status === 'fulfilled' ? ratingsData.value.ratings : { total: 0, average: 0 },
      userRating: ratingsData.status === 'fulfilled' ? ratingsData.value.userRating : null,
      comments: commentsData.status === 'fulfilled' ? commentsData.value : [],
      saveCount: savesData.status === 'fulfilled' ? savesData.value.saveCount : 0,
      userSaved: savesData.status === 'fulfilled' ? savesData.value.userSaved : false
    }
  } catch (error) {
